...
type PriceDraft = Pick<PriceRow, "input" | "output" | "cacheRead" | "cacheWrite">;
...
function formatTrendLabel(ms: number, granularity: Granularity, locale: string): string {
  const options: Intl.DateTimeFormatOptions = granularity === "1d"
    ...
    : granularity === "1h"
      ...
      : { minute: "2-digit", second: undefined, hour: "2-digit", hour12: false };
  ...
  return (
    <section className="panel statistics-trend-panel">
      <div className="statistics-panel-head">
        <h3>{t("trend")}</h3>
        <span className="muted statistics-granularity">{t("granularity")}: {t(granularity === "5m" ? "granularity5m" : granularity === "1h" ? "granularity1h" : "granularity1d")}</span>
      </div>
          ...
          {points.map((point, index) => {
            ...
            return (
              <g key={point.bucketStart}>
                ...
                <title>{`${formatTrendLabel(point.bucketStart, granularity, locale)} · ${t("input")}: ${formatTokens(point.inputTokens, locale as never)} · ${t("cacheHit")}: ${formatTokens(point.cacheReadInputTokens, locale as never)} · ${t("cacheCreate")}: ${formatTokens(point.cacheCreationInputTokens, locale as never)} · ${t("output")}: ${formatTokens(point.outputTokens, locale as never)} · ${t("requests")}: ${point.requests}`}</title>
                {index % labelEvery === 0 && (
...
function QuotaTable({ rows, locale, t }: { rows: QuotaRow[]; locale: string; t: (key: StatisticsTextKey) => string }) {
  return (
        ...
        <div className="tbl-wrap statistics-table-wrap">
          <table className="tbl statistics-table">
            ...
            <tbody>{rows.map((row, index) => (
              <tr key={`${row.provider}-${row.quotaType}-${row.periodEnd}-${index}`}>
                <td><strong>{formatProviderDisplayName(row.provider, ((key: string) => key) as never)}</strong><small>{row.account}</small></td>
                ...
                <td>
                  <div className="statistics-percent-cell"><strong>{Math.round(row.usedPercent)}%</strong><span className="statistics-progress"><i style={{ width: `${Math.max(0, Math.min(100, row.usedPercent))}%` }} /></span></div>
                </td>
                <td className="mono">{formatTokens(row.usedTokens, locale as never)} / {row.estimatedTotalTokens === null ? "—" : formatTokens(row.estimatedTotalTokens, locale as never)}</td>
                <td className="mono">{formatEstimatedUsdValue(row.usedCostUsd, locale as never)} / {row.estimatedTotalCostUsd === null ? "—" : formatEstimatedUsdValue(row.estimatedTotalCostUsd, locale as never)}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
...
function ModelTable({ rows, locale, t }: { rows: ModelRow[]; locale: string; t: (key: StatisticsTextKey) => string }) {
  return (
        ...
        <table className="tbl statistics-table statistics-model-table">
          ...
          <tbody>{rows.map((row, index) => (
            <tr key={`${row.provider}-${row.account}-${row.model}`}>
              <td>{index + 1}</td>
              <td><strong>{formatProviderDisplayName(row.provider, ((key: string) => key) as never)} / {modelLabel(row.model)}</strong>{row.account !== "default" && <small>{row.account}</small>}</td>
              <td className="num">{row.requests.toLocaleString(locale)}</td>
              <td className="num statistics-input-cell">
                <strong>{formatTokens(row.inputTokens, locale as never)}</strong>
                <small><span>{t("cacheHit")}: {formatTokens(row.cacheReadInputTokens, locale as never)}</span><span>{t("cacheCreate")}: {formatTokens(row.cacheCreationInputTokens, locale as never)}</span></small>
              </td>
              <td className="num mono">{formatTokens(row.outputTokens, locale as never)}</td>
              <td className="num mono">{formatEstimatedUsdValue(row.estimatedCostUsd, locale as never)}</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
    ...
    <section className="panel statistics-composition-panel">
      ...
      <div className="statistics-donut-row">
        <svg viewBox="0 0 120 120" className="statistics-donut" aria-label={t("tokenComposition")}>
          ...
          <text x="60" y="56" textAnchor="middle" className="statistics-donut-value">{formatTokens(total, locale as never)}</text>
        ...
        <div className="statistics-donut-legend">{parts.map(part => (
          <div key={part.key}><i className={`statistics-swatch ${part.key}`} /><span>{part.label}</span><strong>{formatTokens(part.value, locale as never)}</strong><small>{total > 0 ? pct(part.value / total) : "0%"}</small></div>
        ))}</div>
      </div>
    </section>
  );
...
function CostOverview({ rows, locale, t }: { rows: CostProviderRow[]; locale: string; t: (key: StatisticsTextKey) => string }) {
  return (
    <section className="panel statistics-cost-panel">
      ...
      <div className="statistics-cost-list">{rows.map(row => (
        <div key={row.provider} className="statistics-cost-row">
          <span>{formatProviderDisplayName(row.provider, ((key: string) => key) as never)}</span>
          <div className="statistics-cost-track"><i style={{ width: `${Math.max(2, row.shareRatio * 100)}%` }} /></div>
          <strong>{formatEstimatedUsdValue(row.estimatedCostUsd, locale as never)}</strong>
...
function PriceModal({ apiBase, open, onClose, t }: { apiBase: string; open: boolean; onClose: () => void; t: (key: StatisticsTextKey) => string }) {
  ...
  const load = useCallback(async () => {
    ...
    setRows(prices);
    setDrafts(Object.fromEntries(prices.map(row => [`${row.provider}\u0000${row.model}`, { input: row.input, output: row.output, cacheRead: row.cacheRead, cacheWrite: row.cacheWrite }])));
  }, [apiBase]);
  ...
  const change = (key: string, field: keyof PriceDraft, value: string) => {
    ...
    setDrafts(current => ({ ...current, [key]: { ...current[key]!, [field]: Number.isFinite(parsed) && parsed >= 0 ? parsed : 0 } }));
  };
  ...
  return (
    <div className="statistics-modal-backdrop" role="presentation" onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}>
      <div className="statistics-modal panel" role="dialog" aria-modal="true" aria-label={t("priceTitle")}>
        <div className="statistics-modal-head"><div><h2>{t("priceTitle")}</h2><p className="muted">{t("priceSubtitle")}</p></div><button type="button" className="btn" onClick={onClose}>{t("close")}</button></div>
        ...
        <div className="tbl-wrap statistics-price-table-wrap">
          <table className="tbl statistics-price-table"><thead><tr>
            ...
            return <tr key={key}>
              <td>{row.provider}</td><td className="mono">{modelLabel(row.model)}</td>
              {(["input", "cacheRead", "cacheWrite", "output"] as const).map(field => <td key={field}><input className="input statistics-price-input" type="number" min="0" step="0.01" value={draft[field]} onChange={event => change(key, field, event.target.value)} /></td>)}
              <td><span className={`statistics-price-source${row.custom ? " custom" : ""}`}>{row.custom ? t("customPrice") : t("officialPrice")}</span></td>
              <td className="statistics-price-actions"><button type="button" className="btn btn-primary" disabled={saving === key} onClick={() => void persist(row)}>{saving === key ? t("saving") : t("save")}</button>{row.custom && <button type="button" className="btn" disabled={saving === key} onClick={() => void persist(row, true)}>{t("resetOfficial")}</button>}</td>
            </tr>;
          })}</tbody></table>
        </div>
      </div>
...
export default function Statistics({ apiBase = API_BASE_DEFAULT }: { apiBase?: string }) {
  ...
  const now = Date.now();
  const [from, setFrom] = useState(() => now - 7 * DAY_MS);
  ...
  }, [apiBase, data, from, provider, t, to]);

  useEffect(() => { void load(); }, [from, provider, to]);
  useEffect(() => {
  ...
  const providerOptions = useMemo(() => [
    ...
    ...(data?.filters.providers ?? []).map(value => ({ value, label: formatProviderDisplayName(value, ((key: string) => key) as never) })),
  ], [data?.filters.providers, t]);
    ...
    <div className="statistics-page">
      <div className="statistics-header">
        ...
        <div className="statistics-toolbar">
          ...
          <Select value={provider} options={providerOptions} onChange={setProvider} label={t("provider")} />
      ...
      {loading && !data ? <div className="panel statistics-loading muted">{t("title")}…</div> : summary && previous && data ? (
        <>
          ...
          <div className="statistics-main-grid">
            ...
            <QuotaTable rows={data.quotas} locale={locale} t={t} />
          </div>
          ...
          <div className="statistics-bottom-grid">
            ...
            <CostOverview rows={data.costsByProvider} locale={locale} t={t} />
          </div>
        </>
      ) : null}
